...
Last updated: 2026-07-08 12:28 IST (Formula v1.2 — single isolated correction to f1's sign, pending submission)
...
| **v1.2 (2026-07-08) — single isolated correction, pending submission** | Exactly one change from v1.1: **f1 (Revolve Balance)'s Base coefficient sign flipped from −0.15 to +0.15**. Nothing else touched (risk exponent still ^3, M1/M2/M3 unchanged, `pctrank(lend_exposure)` still has_lend_line=1-conditional). `Base = 0.35·pctrank(f5) − 0.35·pctrank(f11)³ + 0.15·pctrank(f1) + 0.15·pctrank(lend_exposure\|has_lend_line=1)` | **Pending submission** | User's explicit direction after seeing v1.1's 0.175: change one thing at a time and use the leaderboard itself as the ground-truth signal, since no local labels exist to validate economic assumptions against. Rationale for this specific change: the project's own profitability framework (Section 1) defines Revenue as including interest, and revolve balance is what generates interest income — v1.1's negative weight treated it as a pure risk-cost instead, likely double-penalizing a signal f11 already captures on its own. Post-flip diagnostic: overall `corr(Score, f1)` is still mildly negative (-0.06, down from -0.39) because f1 also correlates indirectly with f11 (Spearman 0.58) which is still heavily penalized — expected, not a bug. Top-20% profile: mean f1=₹1,965 (vs. v1.1's ₹321, population ₹2,467) — softened but not fully reversed, proportional to a single-coefficient change. Isolating this one variable means the next leaderboard score can be attributed cleanly to this change alone |
