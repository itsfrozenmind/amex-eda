...
Last updated: 2026-07-08 12:14 IST (Formula v1.1 executed and submission file populated)
...
| **v1.1 (2026-07-08) — EXECUTED, submission file generated** | Same structure as v1 with 3 data-derived corrections: (1) risk exponent changed **^2 → ^3** per Section 13's cubic curve-fit finding; (2) M2 weight changed **0.05 → 0.0628** (lift-derived); (3) `pctrank(lend_exposure)` now computed **only within `has_lend_line=1` rows** (not the full 500K population) — the concept doesn't apply to charge-card-only customers, so they get 0 contribution from this term rather than a distorted percentile. `Base = 0.35·pctrank(f5) − 0.35·pctrank(f11)³ − 0.15·pctrank(f1) + 0.15·pctrank(lend_exposure\|has_lend_line=1)`. M1 (−0.20) and M3 (−0.05) unchanged from v1, but M1's trigger condition now also requires `has_lend_line=1` explicitly. | **Pending submission** — leaderboard score to be filled in after Unstop submission | Executed exactly per user's locked v1.1 spec (see chat handoff). Notebook Section O has full implementation + sanity checks. **Re-running the M1 danger-zone check under this corrected population reversed Section 13's conclusion**: danger zone is now 6,550 rows (vs. 21,505 under the old full-population definition, much closer to Section 12's original ~4,448-4,805 scale), and **M1 now pulls 26 customers out of the top 20% who would otherwise have crossed** (vs. 0 under the old full-population/^2 version) — the corrected population definition plus the sharper ^3 penalty means M1 is no longer redundant for this specific segment. Top-20% cohort profile matches expectations: mean f5=₹7,680 (vs. population ₹3,448), mean f11≈0.000 (vs. population 0.034), mean f1=₹321 (vs. population ₹2,467) — high-spend, near-zero-risk, low-revolve, as designed. Output: `submission_v1_1.xlsx` (Predictions sheet: 500,000 rows, IDs 0–499,999 sequential, no nulls; Profitability Framework sheet: all 10 sections populated) |
